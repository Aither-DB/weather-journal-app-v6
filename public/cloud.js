// Credit to Dave Kwiatkowski's animated 'The ASCII Galaxy' to make cloud animation at top of page: https://codepen.io/davekwiatkowski/pen/LyvXWK. I've used their original code and replaced the symbols with clouds.
...
window.onload = () => {
	...
	function update() {
		...
		for (let i = 0; i < CHARS.length; i++) {
			const DX = 0.0005 * Math.sin(time * 0.001);
			const DY = 0.0005 * Math.cos(time * 0.001);
			CHARS[i].rotate("x", DX);
			CHARS[i].rotate("y", DY);
		}
		++time;
